...
const mongoose = require('../../common/services/mongoose.service').mongoose;
  ...
  exports.listAll = (req, res ) => {
     ...
     */
    TransactionModel.listAll(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    TransactionModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
   ...
   // filter['productid']= {"in":req.jwt.productId.map(pid => mongoose.Types.ObjectId(pid))};
     ...
     */
    TransactionModel.findById(req.params.transactionId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      TransactionModel.patchTransaction(req.params.transactionId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    TransactionModel.removeById(req.params.transactionId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
