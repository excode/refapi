const ProductnewModel = require('./productnew.model');
  ...
  exports.insert = (req, res) => {
        ...
        req.body.createAt=funcs.getTime()
            ...
            ProductnewModel.createProductnew(req.body)
                  .then((result) => {
  ...
  exports.list = (req, res ) => {
      ...
      ProductnewModel.list(limit, page,req.query)
          .then((result) => {
  ...
  exports.listAll = (req, res ) => {
     ...
     */
    ProductnewModel.listAll(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
...
exports.listSuggestions = (req, res ) => {
     ...
     */
    ProductnewModel.listSuggestions(req.query)
        .then((result) => {
            res.status(200).send(result);
        }).catch((err)=>{
            res.status(400).json( {err:err} );
  ...
  exports.getById = (req, res) => {
     ...
     */
    ProductnewModel.findById(req.params.productnewId,filter)
          .then((result) => {
              res.status(200).send(result);
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.patchById = (req, res) => {
     ...
     */
      ProductnewModel.patchProductnew(req.params.productnewId, req.body,filter)
          .then((result) => {
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
  ...
  exports.removeById = (req, res) => {
     ...
     */
    ProductnewModel.removeById(req.params.productnewId,filter)
          .then((result)=>{
              res.status(204).send({});
          }).catch((err)=>{
              res.status(400).json( {err:err} );
...
exports.uploadfile=(req,res)=>{
    ProductnewModel.uploadFile(req)
        .then((result) => {
